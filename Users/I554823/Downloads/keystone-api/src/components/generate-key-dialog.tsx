...
export function GenerateKeyDialog({ open, onOpenChange, onKeyGenerated }: GenerateKeyDialogProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    toast({
        title: "Gateway API Key Generated",
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="font-headline">Generate New Gateway API Key</DialogTitle>
          <DialogDescription>
            Create a new key to access the gateway services.
          </DialogDescription>
        </DialogHeader>
        {generatedKey ? (
